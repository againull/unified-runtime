...
ur_result_t urProgramBuildExp(
  ...
  for (uint32_t i = 0; i < numDevices; i++) {
    ...
    // We don't need provided binary anymore.
    hProgram->resetBinaryCode(ZeDevice);
  }

  // We no longer need the IL code if program was compiled for all devices in
  // the context. Check if all records in DeviceInfoMap have the exe state.
  if (std::all_of(hProgram->AssociatedDevices.begin(),
                  hProgram->AssociatedDevices.begin(),
                  [&hProgram](const auto &Dev) {
                    return hProgram->getState(Dev->ZeDevice) ==
                           ur_program_handle_t_::Exe;
                  })) {
    hProgram->resetSpirvCode();
  }
  return Result;
}
...
ur_result_t urProgramGetInfo(
  ...
  case UR_PROGRAM_INFO_SOURCE:
    return ReturnValue(Program->getCode());
  default:
